...
mc[2].metric(
    "Users",
    f"{interactions['user_id'].nunique():,}" if interactions is not None else "—",
)
mc[3].metric(
    "Cities",
    (
        businesses["city"].nunique()
        if businesses is not None and "city" in businesses
        else "—"
    ),
)
    ...
    with st.container(border=True):
        ...
        if reviews is not None and "date" in reviews.columns:
            try:
                ...
                monthly["Trend"] = (
                    monthly["Reviews"].rolling(3, min_periods=1).mean().round(0)
                )
                ...
                fig_ts.add_trace(
                    go.Scatter(
                        ...
                        y=monthly["Reviews"],
                        fill="tozeroy",
                        fillcolor="rgba(255,122,26,0.13)",
                ...
                st.plotly_chart(
                    fig_ts, use_container_width=True, config={"displayModeBar": False}
                )
                _ts_built = True
            except Exception:
                pass
        ...
        if (
            ...
            and "date" in interactions.columns
        ):
            try:
                ...
                monthly = monthly.sort_values("Month").tail(24)
                monthly["Trend"] = (
                    monthly["Count"].rolling(3, min_periods=1).mean().round(0)
                )

                _card_title(
                    "Interactions over time", "monthly volume · trailing 24 months"
                )
                fig_ts = go.Figure()
                fig_ts.add_trace(
                    go.Scatter(
        ...
        if not _ts_built:
            ...
            _card_title(
                "Review volume by star rating", "distribution across all reviews"
            )
...
with r2c1:
    with st.container(border=True):
        ...
        if businesses is not None and "categories" in businesses.columns:
            ...
            st.plotly_chart(
                fig_cuis, use_container_width=True, config={"displayModeBar": False}
            )
        else:
...
with r2c2:
    with st.container(border=True):
        if (
            ...
            agg = (
                reviews.groupby("stars")["sentiment_compound"]
                .mean()
                .round(3)
                .reset_index()
            )
            ...
            fig_sent = go.Figure(
                go.Bar(
                    ...
                    y=agg["Sentiment"],
                    marker_color=[
                        _ACCENT if i == peak_idx else _GRAY for i in range(len(agg))
                    ],
                    marker_line_width=0,
            ...
            st.plotly_chart(
                fig_sent, use_container_width=True, config={"displayModeBar": False}
            )
            ...
            fig_stars = go.Figure(
                go.Bar(
                    ...
                    y=sc.values,
                    marker_color=[
                        _ACCENT if i == peak_idx else _GRAY for i in range(len(sc))
                    ],
                    marker_line_width=0,
            ...
            st.plotly_chart(
                fig_stars, use_container_width=True, config={"displayModeBar": False}
            )
        else:
    ...
    with _ec1:
        if st.button("▶ Run Evaluation Now", type="primary"):
            ...
            if not has_trained_models():
                st.error(
                    "No trained models found. Run the Train stage from Admin first."
                )
            else:
                try:
                    with st.spinner("Evaluating models — this may take a minute …"):
                        ...
                        if "interactions" not in _d or "businesses" not in _d:
                            st.error(
                                "Processed data missing. Run the preprocessing stage first."
                            )
                            st.stop()
                        _models = {
                            k: load_pickle(MODELS_DIR / MODEL_FILES[k])
                            for k in (
                                "popularity",
                                "content_based",
                                "item_cf",
...
if eval_path.exists():
    ...
    ranking_cols = [
        c for c in eval_df.columns if c.startswith(("precision", "recall", "ndcg"))
    ]
        ...
        with mc2:
            with st.container(border=True):
                if error_cols:
                    ...
                    prec_col = next(
                        (c for c in eval_df.columns if c.startswith("precision")), None
                    )
                    rec_col = next(
                        (c for c in eval_df.columns if c.startswith("recall")), None
                    )
                    ndcg_col = next(
                        (c for c in eval_df.columns if c.startswith("ndcg")), None
                    )
                    if prec_col and rec_col:
                        _card_title(
                            "Precision–Recall", "trade-off · bubble size = nDCG"
                        )
    ...
    if ranking_cols:
        with st.container(border=True):
            ...
            rank_tbl = (
                eval_df[["Model"] + ranking_cols + error_cols].set_index("Model").copy()
            )
...
if interactions is not None and not interactions.empty:
        ...
        with st.container(border=True):
            ...
            fig_act = go.Figure(
                go.Bar(
                    ...
                    y=bucket.values,
                    marker_color=[
                        _ACCENT if i == 0 else _GRAY for i in range(len(bucket))
                    ],
                    marker_line_width=0,
                    text=bucket.values,
                    textposition="outside",
            ...
            st.plotly_chart(
                fig_act, use_container_width=True, config={"displayModeBar": False}
            )
    ...
    with ac2:
        with st.container(border=True):
            ...
            sparsity = 1 - (
                len(interactions)
                / (per_user.size * interactions["business_id"].nunique())
            )
            stats_html = ""
            items = [
