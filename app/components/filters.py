...
def render_filter_sidebar(
    ...
    with st.expander("Refine results", expanded=False):
        ...
        with fc1:
            ...
            radius_km = st.slider(
                "Radius (km)", min_value=0.5, max_value=25.0,
                value=float(default_radius), step=0.5,
            )
        with fc2:
            min_stars = st.slider(
                "Min stars", min_value=1.0, max_value=5.0, value=3.5, step=0.5,
            )
            price_levels = st.multiselect(
                "Price levels", options=[1, 2, 3, 4], default=[1, 2, 3, 4],
...
def render_filter_rail(
    ...
    cuisines = st.multiselect(
        "Cuisines", options=cuisine_options, default=[],
        label_visibility="collapsed", key="fr_cuisines",
    )

    st.markdown('<span class="disc-flabel">Radius (km)</span>', unsafe_allow_html=True)
    radius_km = st.slider(
        "Radius", min_value=0.5, max_value=25.0,
        value=float(default_radius), step=0.5,
        label_visibility="collapsed", key="fr_radius",
    )
    ...
    min_stars = st.slider(
        "Min stars", min_value=1.0, max_value=5.0, value=3.5, step=0.5,
        label_visibility="collapsed", key="fr_min_stars",
    )

    st.markdown('<span class="disc-flabel">Price</span>', unsafe_allow_html=True)
    price_levels = st.multiselect(
        "Price", options=[1, 2, 3, 4], default=[1, 2, 3, 4],
        format_func=lambda x: "$" * x,
        label_visibility="collapsed", key="fr_price",
    )
    ...
    top_n = st.slider(
        "Top-N", min_value=3, max_value=20, value=8,
        label_visibility="collapsed", key="fr_top_n",
    )
    open_now = st.checkbox("Open now", value=False, key="fr_open_now")
    show_components = st.checkbox("Score breakdown", value=False, key="fr_show_components")
